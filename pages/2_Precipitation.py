"""
GPM IMERG Precipitation Trend — Interactive GeoTIFF Dashboard
==============================================================
Install:
    pip install streamlit plotly rasterio numpy pandas matplotlib scipy

Run (standalone):
    streamlit run 6_GPM_Precipitation.py
...
# ═══════════════════════════════════════════════════════════════════════════════
st.set_page_config(
    page_title="GPM Precipitation Trend",
    page_icon="🌧️",
    layout="wide",
...
def make_zonal_bar(data, lats, title="Zonal Mean Precip Trend"):
...
def make_longitudinal_bar(data, lons, title="Longitudinal Mean Precip Trend"):
...
# Header
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown("# 🌧️ GPM Precipitation Trend Explorer")
st.markdown(
    "<span style='font-family:IBM Plex Mono;font-size:12px;color:#8b949e;'>"
    "Interactive analysis of long-term precipitation trends"
    "&nbsp;·&nbsp; mm/year &nbsp;·&nbsp; NASA GPM IMERG 2001–2023"
    "</span>",
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "Source: NASA GPM IMERG<br>Precipitation trend 2001–2023"
        "</span>",
        unsafe_allow_html=True,
    )
...
col1.metric("Mean trend",     f"{np.nanmean(valid_all):+.4f}",    "mm/yr")
col2.metric("Median trend",   f"{np.nanmedian(valid_all):+.4f}",  "mm/yr")
col3.metric("Std dev",        f"{np.nanstd(valid_all):.4f}",      "mm/yr")
col4.metric("Max gain",       f"{np.nanmax(valid_all):+.4f}",     "mm/yr")
col5.metric("Max loss",       f"{np.nanmin(valid_all):+.4f}",     "mm/yr")
col6.metric("Wetter pixels",  f"{pct_wetter:.1f}%",
...
# ── Tab 1: Map ────────────────────────────────────────────────────────────────
with tab_map:
    map_title = f"GPM Precipitation Trend 2001–2023 — {region_name}"
    ...
    if region_name != "Global":
        ...
        rc1.metric(f"{region_name} — Mean",   f"{np.nanmean(valid_crop):+.4f} mm/yr")
        rc2.metric(f"{region_name} — Median", f"{np.nanmedian(valid_crop):+.4f} mm/yr")
        rc3.metric(f"{region_name} — Max",    f"{np.nanmax(valid_crop):+.4f} mm/yr")
        rc4.metric(f"{region_name} — Min",    f"{np.nanmin(valid_crop):+.4f} mm/yr")

    st.caption(
        f"CRS: {raster['crs']} · "
        f"Native resolution: {raster['orig_shape'][1]}×{raster['orig_shape'][0]} px · "
        f"Display downsampled ×{downsample} · "
        f"Colour range: [{vmin:.4f}, {vmax:.4f}] mm/yr"
    )
...
with tab_hist:
    ...
    pct_df   = pd.DataFrame({
        ...
        "Value (mm/yr)": [f"{v:+.5f}" for v in pct_vals],
...
with tab_zonal:
    ...
    fig_zonal = make_zonal_bar(data_display, lats_crop,
                               title="Mean Precip Trend by Latitude Band")
    ...
    fig_lon = make_longitudinal_bar(data_display, lons_crop,
                                    title="Mean Precip Trend by Longitude Band")
...
with tab_stats:
    ...
    stat_rows = [
        ("Mean",               f"{np.nanmean(valid_all):+.6f}",   "mm/yr"),
        ("Median",             f"{np.nanmedian(valid_all):+.6f}", "mm/yr"),
        ("Std deviation",      f"{np.nanstd(valid_all):.6f}",     "mm/yr"),
        ("Min (driest trend)", f"{np.nanmin(valid_all):+.6f}",    "mm/yr"),
        ("Max (wettest trend)",f"{np.nanmax(valid_all):+.6f}",    "mm/yr"),
    ...
    if region_name != "Global":
        ...
        region_rows = [
            ("Mean",     f"{np.nanmean(valid_crop):+.6f}",   "mm/yr"),
            ("Median",   f"{np.nanmedian(valid_crop):+.6f}", "mm/yr"),
            ("Std",      f"{np.nanstd(valid_crop):.6f}",     "mm/yr"),
            ("Min",      f"{np.nanmin(valid_crop):+.6f}",    "mm/yr"),
            ("Max",      f"{np.nanmax(valid_crop):+.6f}",    "mm/yr"),
...
with tab_export:
    ...
    with col_a:
        ...
        st.caption("Exports a flat table of lon, lat, precipitation trend value for the current region.")
