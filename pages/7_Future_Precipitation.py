"""
Precipitation Slope — SSP2-4.5 vs SSP5-8.5 Interactive Dashboard
=================================================================
Install:
    pip install streamlit plotly rasterio numpy pandas matplotlib scipy

Run:
    streamlit run precip_ssp_dashboard.py
...
# ═══════════════════════════════════════════════════════════════════════════════
st.set_page_config(
    page_title="Precipitation SSP2-4.5 vs SSP5-8.5",
    page_icon="🌧️",
    layout="wide",
...
DATASETS = {
    "ssp245": {
        ...
        "label"     : "SSP2-4.5",
        "long_label": "Precipitation Slope — SSP2-4.5",
        "color"     : "#38bdf8",
        "units"     : "mm/day/yr",
    },
    "ssp585": {
        ...
        "label"     : "SSP5-8.5",
        "long_label": "Precipitation Slope — SSP5-8.5",
        "color"     : "#f87171",
        "units"     : "mm/day/yr",
    },
}
...
def diff_fig(d245, d585, lons, lats, units, height=480):
    ...
    fig.update_layout(
        **layout_base(height),
        title=dict(text="Difference: SSP5-8.5 − SSP2-4.5  (red = wetter, blue = drier)",
...
def hist_fig(v245, v585, vmin, vmax, cmap_name, units, bins=70):
    ...
    fig.update_layout(
        barmode="overlay",
        xaxis=dict(**ax_style(f"Precipitation Slope ({units})")),
...
def zonal_fig(d245, d585, lats):
    ...
    fig.update_layout(
        **layout_base(440),
        ...
        xaxis=dict(**ax_style("Mean Slope (mm/day/yr)")),
...
def longitudinal_fig(d245, d585, lons):
    ...
    fig.update_layout(
        **layout_base(300),
        ...
        yaxis=dict(**ax_style("Mean Slope (mm/day/yr)")),
    )
    return fig
...
def zonal_diff_fig(d245, d585, lats):
    ...
    fig.update_layout(
        **layout_base(440),
        ...
        xaxis=dict(**ax_style("Slope difference (mm/day/yr)")),
        yaxis=dict(**ax_style("Latitude")),
    )
    return fig
    ...
    fig.update_layout(
        **layout_base(420),
        title=dict(text=f"Pixel-wise scatter: SSP2-4.5 vs SSP5-8.5  (r = {r:.4f})",
                   font=dict(family="JetBrains Mono", size=12, color=TEXT), x=0.01),
        xaxis=dict(**ax_style("SSP2-4.5 slope (mm/day/yr)")),
        yaxis=dict(**ax_style("SSP5-8.5 slope (mm/day/yr)")),
    )
    return fig, r

def wetdry_comparison_fig(v245, v585):
    """Grouped bar: % wetter / drier / unchanged for each scenario."""
    rows = []
    for v, label in [(v245, "SSP2-4.5"), (v585, "SSP5-8.5")]:
        n = len(v)
        rows.append({
            "Scenario" : label,
            "Wetter ↑" : 100 * np.mean(v > 0),
            "Drier ↓"  : 100 * np.mean(v < 0),
            "No change": 100 * np.mean(v == 0),
        })
    df = pd.DataFrame(rows)

    fig = go.Figure()
    for col, color in [("Wetter ↑", "#4ade80"), ("Drier ↓", "#f87171"), ("No change", MUTED)]:
        fig.add_trace(go.Bar(
    ...
    fig.update_layout(
        ...
        **layout_base(320),
        title=dict(text="Wetter / Drier / Unchanged Pixel Split by Scenario",
                   font=dict(family="JetBrains Mono", size=12, color=MUTED)),
    )
    return fig
...
# Header
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown("# 🌧️ Precipitation Slope — SSP2-4.5 vs SSP5-8.5")
st.markdown(
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "Source: CMIP6 via GEE<br>Scenario: SSP2-4.5 · SSP5-8.5<br>Variable: Precipitation slope"
...
if len(v245):
    ...
    m2.metric("SSP2-4.5 Wetter", f"{100*np.mean(v245>0):.1f}%",
              f"{100*np.mean(v245<0):.1f}% drier")
...
if len(v585):
    ...
    m6.metric("SSP5-8.5 Wetter", f"{100*np.mean(v585>0):.1f}%",
              f"{100*np.mean(v585<0):.1f}% drier")
        ...
        if len(vals):
            ...
            with col:
                ...
                st.dataframe(pd.DataFrame({
                    ...
                    "Dir"  : ["↑ Wetter" if x > 0 else "↓ Drier" for x in pv],
...
with tab_zonal:
    if d245 is not None and d585 is not None:
        ...
        st.caption("Red = SSP5-8.5 wetter than SSP2-4.5 · Blue = SSP5-8.5 drier than SSP2-4.5")
    else:
...
# ── Wet / Dry breakdown ───────────────────────────────────────────────────────
with tab_wetdry:
    st.markdown('<div class="section-header">Wetter / Drier / Unchanged by Scenario</div>',
                unsafe_allow_html=True)
    if len(v245) and len(v585):
        st.plotly_chart(wetdry_comparison_fig(v245, v585), use_container_width=True)
        ...
        for col, vals, label in [(wdc1, v245, "SSP2-4.5"), (wdc2, v585, "SSP5-8.5")]:
            ...
            with col:
                st.markdown(f"**{label}**")
                df_wd = pd.DataFrame([
                    {"Category": "Wetter ↑",  "Pixels": n_wet,  "Pct": f"{100*n_wet/total:.2f}%",
                     "Mean slope": f"{np.mean(vals[vals>0]):+.7f}" if n_wet else "N/A"},
                    {"Category": "Drier ↓",   "Pixels": n_dry,  "Pct": f"{100*n_dry/total:.2f}%",
...
with tab_stats:
    for k, vals in [("ssp245", v245), ("ssp585", v585)]:
        ...
        rows = [
            ...
            ("% Wetter ↑",    f"{100*np.mean(vals>0):.3f}",  "%"),
            ("% Drier ↓",     f"{100*np.mean(vals<0):.3f}",  "%"),
        ...
        if region_name != "Global":
            rows += [
                ...
                (f"[{region_name}] Wetter", f"{100*np.mean(vc>0):.2f}", "%"),
                (f"[{region_name}] Drier",  f"{100*np.mean(vc<0):.2f}", "%"),
            ]
...
with tab_export:
    ...
    with ec1:
        ...
        export_df = pd.DataFrame({
            ...
            f"pr_slope_{units.replace('/','_')}": exp_data.ravel(),
        }).dropna()
        st.download_button(
            "⬇️ Download CSV",
            data      = export_df.to_csv(index=False).encode("utf-8"),
            file_name = f"pr_{exp_key}_{region_name.lower().replace(' ','_')}.csv",
            mime      = "text/csv",
        )
    ...
    with ec2:
        ...
        if st.button("Render PNG"):
            ...
            if sym:
                ...
                ev_min, ev_max = -e, e
            cmap_obj = plt.get_cmap("RdBu" if exp_key == "diff" else cmap_name)
            ...
            st.download_button(
                ...
                file_name=f"pr_{exp_key}_{region_name.lower().replace(' ','_')}.png",
                mime="image/png",
            )

st.markdown("---")
st.caption("Precipitation slope · CMIP6 · SSP2-4.5 & SSP5-8.5 · Source: GEE export")
