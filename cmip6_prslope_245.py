"""
Precipitation Slope SSP2-4.5 — Interactive GeoTIFF Dashboard
=============================================================
Install:
...
# ═══════════════════════════════════════════════════════════════════════════════
st.set_page_config(
    page_title="Precipitation Slope SSP2-4.5",
    page_icon="🌧️",
    layout="wide",
...
# Header
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown("# 🌧️ Precipitation Slope — SSP2-4.5")
st.markdown(
    "<span style='font-family:Space Mono;font-size:11px;color:#6e8aab;'>"
    "Interactive exploration of projected precipitation trend slope &nbsp;·&nbsp; SSP2-4.5 scenario"
    "</span>",
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "Scenario: SSP2-4.5<br>Variable: Precipitation slope<br>"
...
with tab_map:
    fig_map = make_heatmap(
        ...
        title=f"Precipitation Slope (SSP2-4.5) — {region_name}",
    )
...
with tab_export:
    ...
    with col_a:
        ...
        st.download_button(
            "⬇️ Download CSV",
            data      = csv_bytes,
            file_name = f"pr_slope_ssp245_{region_name.lower().replace(' ','_')}.csv",
            mime      = "text/csv",
        )
    ...
    with col_b:
        ...
        if st.button("Generate PNG"):
            ...
            ax.set_title(
                f"Precipitation Slope SSP2-4.5 — {region_name}",
            ...
            st.download_button(
                ...
                file_name=f"pr_slope_ssp245_{region_name.lower().replace(' ','_')}.png",
                mime="image/png",
            )
            st.success("PNG ready — click above to save.")

st.markdown("---")
st.caption("Scenario: SSP2-4.5 · Variable: Precipitation slope · Source: CMIP6 via GEE")
